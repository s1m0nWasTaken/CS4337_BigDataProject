...
public class OrderService {
  ...
  public ResponseEntity<Map<String, Object>> addOrder(@RequestBody Order order) {
    try {
      ...
      transaction.setTransactionStatus(
          order.getOrderStatus().toString()); // Aligning order status with transaction status
      ...
      ResponseEntity<Map> transactionResponse =
          restTemplate.postForEntity(transactionUrl, transaction, Map.class);
      ...
      if (transactionResponse.getStatusCode() == HttpStatus.OK
        ...
        if (responseBody.containsKey("transactionId")) {
          ...
          String sqlInsert =
              "INSERT INTO Orders (orderStatus, deliveryAddress, shopItemid, transactionid, price) VALUES (?, ?, ?, ?, ?)";
          jdbcTemplate.update(
              sqlInsert,
          ...
          return ResponseEntity.status(HttpStatus.BAD_REQUEST)
              .body(Map.of("error", "Transaction failed, order not created"));
        }
      } else {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
      ...
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
          .body(Map.of("error", "Failed to connect to transaction service: " + e.getMessage()));
    }
  }
  ...
  public ResponseEntity<Map<String, Object>> getOrders(
    ...
    // TODO need to add checking that it only returns all if user is admin and returns specified
    // ones for normal user

    // TODO additional checking for shop owners needed too - logic is to join order -> shopitem ->
    // shop and get all orders related to your shop
    try {
